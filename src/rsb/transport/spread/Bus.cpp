...
BusPtr Bus::create(SpreadConnectionPtr connection) {
    return BusPtr(new Bus(connection));
}

Bus::Bus(SpreadConnectionPtr connection) :
...
void Bus::addSink(const Scope& scope, SinkPtr sink) {
...
void Bus::removeSink(const Scope& scope, const Sink* sink) {
    RSCDEBUG(this->logger,
...
void Bus::handleOutgoingNotification(OutgoingNotificationPtr notification) {
...
void Bus::handleIncomingNotification(IncomingNotificationPtr notification) {
...
void Bus::handleError(const std::exception& error) {
    boost::mutex::scoped_lock lock(this->sinkMutex);
...
void Bus::sendNotification(OutgoingNotificationPtr notification) {
