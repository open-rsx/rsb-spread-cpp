...
Assembly::Assembly(rsb::protocol::FragmentedNotificationPtr n) :
    logger(Logger::getLogger(str(format("rsb.transport.spread.Assembly[%1%]")
...
bool Assembly::add(FragmentedNotificationPtr n) {
    ...
    if (store[n->data_part()]) {
        throw ProtocolException(boost::str(boost::format("Received fragment (%d/%d) of notification for event with sender id %x and sequence number %d twice!.")
